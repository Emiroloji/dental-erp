        ...
        <div class="min-h-screen lg:flex" x-data="{ menuOpen: false }" x-on:keydown.escape.window="menuOpen = false">
            ...
            <aside class="bg-panel-900 text-white fixed inset-y-0 left-0 z-40 w-72 overflow-y-auto flex flex-col -translate-x-full transition-transform duration-200 lg:static lg:z-auto lg:w-64 lg:shrink-0 lg:translate-x-0 lg:overflow-visible"
                ...
                <nav class="flex-1 px-3 py-2 space-y-0.5">
                    ...
                    @php($newLeads = \App\Domain\Platform\Models\Lead::where('status', 'new')->count())
                    <x-nav-link :href="route('platform.leads.index')" :active="request()->routeIs('platform.leads.*')">
                        <x-slot:icon><path d="M4 6h16v12H4V6Zm0 0 8 6 8-6" stroke-linecap="round" stroke-linejoin="round"/></x-slot:icon>
                        Gelen Talepler
                        @if ($newLeads > 0)
                            <span class="ml-1 min-w-[18px] h-[18px] px-1 rounded-full bg-status-warn text-white text-[11px] leading-[18px] text-center">{{ $newLeads }}</span>
                        @endif
                    </x-nav-link>
